#!/usr/bin/env bash
# doctor.sh — preflight health check for an ai-sdlc setup.
#
...
#   • the pipeline scripts are installed under .sdlc/scripts (run /sdlc:init)
#
...
echo "ai-sdlc doctor — checking $(pwd)"
echo
...
# 5. .sdlc/scripts installed ------------------------------------------------
if [[ -d "${SDLC_DIR}/scripts" ]]; then
  pass "pipeline scripts installed (${SDLC_DIR}/scripts)"
else
  fail "pipeline scripts not installed (${SDLC_DIR}/scripts missing)" "run /sdlc:init in this repo"
fi
...
if [[ $FAIL -eq 0 ]]; then
  ...
  sdlc_log doctor result=pass passed="$PASS" total="$total" 2>/dev/null || true
  exit 0
else
  printf 'Summary: %d/%d checks passed, %d failed \xe2\x80\x94 fix the \xe2\x9c\x98 items above.\n' "$PASS" "$total" "$FAIL"
  sdlc_log doctor result=fail passed="$PASS" failed="$FAIL" total="$total" 2>/dev/null || true
  exit 1
fi
