...
case "$cmd" in
  add)
    ...
    bid="$(sdlc_issue_id "$blk")"; [[ -n "$bid" ]] || sdlc_die "could not resolve DB id for #$blk"
    gh api --method POST "repos/${repo}/issues/${issue}/dependencies/blocked_by" -F "issue_id=${bid}" >/dev/null \
      && sdlc_log dependency-add issue="$issue" blocker="$blk" \
  ...
  remove)
    ...
    ;;
  *) sdlc_die "usage: dependency.sh <add|list|open|remove> ..." ;;
esac
