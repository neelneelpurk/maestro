...
if ! ( cd "$workdir" 2>/dev/null && bash "$gate" --no-install >&2 ); then
  echo "maestro: quality gate failed in ${workdir} — blocking 'gh pr create'. Fix the failures before opening a PR (or open it through the pipeline, which reports gate output)." >&2
