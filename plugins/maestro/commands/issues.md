...
Break PRD #`$ARGUMENTS` into independently-grabbable issues, using the aligned `CONTEXT.md`/ADR vocabulary. (Scripts live in `.maestro/scripts/`.)
...
Do not modify or close the PRD. When done, recommend `/maestro:drain` (or `/maestro:ship <issue>` for one).
