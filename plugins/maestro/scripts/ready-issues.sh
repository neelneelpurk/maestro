...
# issues that are open, assigned to me (MAESTRO_ASSIGNEE, default @me), labelled
...
out="--json"; assignee_args=(--assignee "$MAESTRO_ASSIGNEE")
for a in "$@"; do
  case "$a" in
...
merged="$(jq -s 'add | unique_by(.number)' <(q "$MAESTRO_LABEL_READY_AGENT") <(q "$MAESTRO_LABEL_AUTO"))"

# Drop in-flight / closure / hitl / parent states.
filtered="$(jq -c \
  --arg ip "$MAESTRO_LABEL_IN_PROGRESS" --arg ir "$MAESTRO_LABEL_IN_REVIEW" \
  --arg wc "$MAESTRO_LABEL_WAITING_CLOSURE" --arg hitl "$MAESTRO_LABEL_HITL" \
  --arg prd "$MAESTRO_LABEL_PRD" --arg roadmap "$MAESTRO_LABEL_ROADMAP" '
...
ready="$(
  jq -c '.[]' <<<"$filtered" | while IFS= read -r obj; do
    ...
    if [[ -z "$(bash "${SCRIPT_DIR}/blocked-by.sh" "$n" 2>/dev/null)" ]]; then
      ...
      jq -c --arg slug "$(maestro_slug "$title")" '{number, title, labels: (.labels|map(.name)), slug: $slug}' <<<"$obj"
    fi
