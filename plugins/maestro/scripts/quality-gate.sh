...
#
# Detection is overridable per-repo via .maestro/config.sh (or env):
#   MAESTRO_INSTALL_CMD, MAESTRO_LINT_CMD, MAESTRO_TYPECHECK_CMD, MAESTRO_TEST_CMD
...
[[ -n "${MAESTRO_INSTALL_CMD+x}" ]]   && install_cmd="$MAESTRO_INSTALL_CMD"
[[ -n "${MAESTRO_LINT_CMD+x}" ]]      && lint_cmd="$MAESTRO_LINT_CMD"
[[ -n "${MAESTRO_TYPECHECK_CMD+x}" ]] && typecheck_cmd="$MAESTRO_TYPECHECK_CMD"
[[ -n "${MAESTRO_TEST_CMD+x}" ]]      && test_cmd="$MAESTRO_TEST_CMD"
...
if [[ ${#FAILED[@]} -gt 0 ]]; then
  ...
  maestro_log quality-gate result=fail steps="${FAILED[*]}" 2>/dev/null || true
  exit 1
fi
if [[ $RAN -eq 0 ]]; then
