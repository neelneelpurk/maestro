...
5. **Post your plan + decomposition to the issue** (so the work is reviewable as it happens):
   ...
   - [ ] step 2
   ```
6. **Implement strictly test-first** ([TDD.md](TDD.md)): red → green → refactor, one step at a time, until the acceptance criteria are met. Match the codebase's existing style, naming, and structure; keep it a thin vertical slice; follow `.claude/rules/maestro.md`.
